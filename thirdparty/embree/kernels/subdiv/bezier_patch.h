...
{  
  ...
  template<typename Vertex>
    __forceinline Vertex computeInnerBezierControlePoint(const Vertex v[4][4], const size_t y, const size_t x) {
    return 1.0f / 36.0f * (16.0f * v[y][x] + 4.0f * (v[y-1][x] +  v[y+1][x] + v[y][x-1] + v[y][x+1]) + (v[y-1][x-1] + v[y+1][x+1] + v[y-1][x+1] + v[y+1][x-1]));
  }
  
  template<typename Vertex>
    __forceinline Vertex computeTopEdgeBezierControlePoint(const Vertex v[4][4], const size_t y, const size_t x) {
    return 1.0f / 18.0f * (8.0f * v[y][x] + 4.0f * v[y-1][x] + 2.0f * (v[y][x-1] + v[y][x+1]) + (v[y-1][x-1] + v[y-1][x+1]));
  }

  template<typename Vertex>
    __forceinline Vertex computeBottomEdgeBezierControlePoint(const Vertex v[4][4], const size_t y, const size_t x) {
    return 1.0f / 18.0f * (8.0f * v[y][x] + 4.0f * v[y+1][x] + 2.0f * (v[y][x-1] + v[y][x+1]) + v[y+1][x-1] + v[y+1][x+1]);
  }
  
  template<typename Vertex>
    __forceinline Vertex computeLeftEdgeBezierControlePoint(const Vertex v[4][4], const size_t y, const size_t x) {
    return 1.0f / 18.0f * (8.0f * v[y][x] + 4.0f * v[y][x-1] + 2.0f * (v[y-1][x] + v[y+1][x]) + v[y-1][x-1] + v[y+1][x-1]);
  }
  
  template<typename Vertex>
    __forceinline Vertex computeRightEdgeBezierControlePoint(const Vertex v[4][4], const size_t y, const size_t x) {
    return 1.0f / 18.0f * (8.0f * v[y][x] + 4.0f * v[y][x+1] + 2.0f * (v[y-1][x] + v[y+1][x]) + v[y-1][x+1] + v[y+1][x+1]);
  }
  
  template<typename Vertex>
    __forceinline Vertex computeCornerBezierControlePoint(const Vertex v[4][4], const size_t y, const size_t x, const ssize_t delta_y, const ssize_t delta_x)
  {
  ...
  public:
    ...
    __forceinline BezierPatchT(const BSplinePatchT<Vertex,Vertex_t>& source)
    {
      ...
      matrix[0][0] = computeInnerBezierControlePoint(source.v,1,1);
      matrix[0][3] = computeInnerBezierControlePoint(source.v,1,2);
      matrix[3][3] = computeInnerBezierControlePoint(source.v,2,2);
      matrix[3][0] = computeInnerBezierControlePoint(source.v,2,1);
      
      /* compute top edge control points */
      matrix[0][1] = computeRightEdgeBezierControlePoint(source.v,1,1);
      matrix[0][2] = computeLeftEdgeBezierControlePoint(source.v,1,2); 
      
      /* compute bottom edge control points */
      matrix[3][1] = computeRightEdgeBezierControlePoint(source.v,2,1);
      matrix[3][2] = computeLeftEdgeBezierControlePoint(source.v,2,2);
      
      /* compute left edge control points */
      matrix[1][0] = computeBottomEdgeBezierControlePoint(source.v,1,1);
      matrix[2][0] = computeTopEdgeBezierControlePoint(source.v,2,1);
      
      /* compute right edge control points */
      matrix[1][3] = computeBottomEdgeBezierControlePoint(source.v,1,2);
      matrix[2][3] = computeTopEdgeBezierControlePoint(source.v,2,2);
      
      /* compute corner control points */
      matrix[1][1] = computeCornerBezierControlePoint(source.v,1,1, 1, 1);
      matrix[1][2] = computeCornerBezierControlePoint(source.v,1,2, 1,-1);
      matrix[2][2] = computeCornerBezierControlePoint(source.v,2,2,-1,-1);
      matrix[2][1] = computeCornerBezierControlePoint(source.v,2,1,-1, 1);      
    }
